...
• طبیعی‌بودن (${CRITIC_RUBRIC.naturalness}): مثل یک آدم واقعی نوشته شده یا مثل تبلیغات؟ وجود هر یک از این عبارت‌های قالبی نمره را کف می‌آورد: ${CLICHE_PHRASES.slice(0, 8).join("، ")}.
  سه نشانه‌ی عینی که هرکدام دیدی از این معیار کم کن:
  الف) **ساختار سه‌تایی** — «نه تنها… بلکه…»، سه صفت پشت‌سرهم، یا سه جمله‌ی متقارن هم‌شکل.
  ب) **مترادف‌بازی** — یک مفهوم واحد که هر بار با کلمه‌ی تازه‌ای گفته شده («محتوا… مطالب… مطلب… کانتنت»). تکرار همان کلمه طبیعی‌تر است.
  ج) **جمله‌های هم‌طول** — وقتی سه جمله‌ی پیاپی تقریباً هم‌اندازه‌اند، متن ریتم ماشینی دارد.
